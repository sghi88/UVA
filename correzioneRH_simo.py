...
dati_CSV = np.array(read_CSV3("OPC2_000.CSV",16))
...
#ma al posto di sostituire i dati, aggiungo 3 colonne alla fine. 
dati_CSV = np.append(dati_CSV, dati_CSV[:, -4:-1]/(1. + (0.4/1.65)/(-1. + 100/np.repeat(dati_CSV[:, -1],3).reshape((len(dati_CSV), 3)))), axis=1)
#quindi avremo: -7 -6 -5 sono i non corretti. -4 HR e poi abbiamo i corretti:  -3 -2 -1

#429  a 0 metto il primo valore HR a 429 il secondo a 858 il terzo e così via
...
ax2.plot(time, dati_CSV[:,-3],alpha=0.6)
#plt.ylim(0,150)
